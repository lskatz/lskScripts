...
$file_name = $ARGV[0] || "seq";


#------------------------------
@nt = ("A","T","G","C");
$J = @nt; #total number of nucleotides
$W = 6; #motif length
#$N = $total_score = $i = 0;
$i=0;
$L = 24; # sequence length - 1
$J = 4; #total number of nucleotides
$total_reps = 10000; #total number of times to go through list of sequences
$check_freq=1000; #how often to compare Fs to see if it should quit
#------------------------------
#minor initializing of variables
if($check_freq>$total_reps){
  $check_freq=$total_reps;
}
...
#--------------------------------------------
    open (IN, "$file_name") || die "Couldn't open input file, $!\n";
...
while (<IN>) {
   ...
   #motif positional counts and background counts
    ...
    $c{$nucleotide}++; # keep track of total nucleotides
