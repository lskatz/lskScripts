...
echo "$QSTAT" | perl -lane 'BEGIN{print "USER\tSLOTS";} next if(!$F[3] || !$F[8]); next if($F[4] ne "r"); $slot{$F[3]}+=$F[8]; END{@user=sort{$slot{$b}<=>$slot{$a} || $a cmp $b} keys(%slot); print "$_\t$slot{$_}" for @user;}' | column -t
