...
QSTAT=$(qstat -u '*' | tail -n +3); # qstat, minus the header
if [ $? -gt 0 ]; then
...
echo "$QSTAT" | perl -lane 'BEGIN{print "USER\tSLOTS";} next if(!$F[3] || !$F[8]); next if($F[4] ne "r"); $slot{$F[3]}+=$F[8]; END{@user=sort{$slot{$b}<=>$slot{$a}} keys(%slot); print "$_\t$slot{$_}" for @user;}' | head|column -t
